...
(function () {
  const audio = new Audio("assets/apache.mp3"); // путь к файлу
  audio.loop = true;
  audio.volume = 1.0;
  ...
  function tryPlay() {
    audio.play()
      .then(() => {
        console.log("Apache music started ✅");
        // как только музыка пошла — больше не слушаем события
  ...
  // слушаем все возможные действия юзера
  ["scroll", "click", "keydown", "mousemove", "touchstart"].forEach(evt => {
